...
namespace nurbs
{
	...
	{
		// Note that this fills the vector with p + 1 zeros and must be replaced in your implementation.
		N.resize(p + 1U, 0.0f);
	}
	...
	glm::vec4 point_on_curve_in_homogeneous_space(std::uint32_t const k, std::vector<float> const& N, std::vector<glm::vec4> const& P, std::uint32_t const p)
	{
		return glm::vec4{0.0};
	}
	...
	///         - Hint: The relevant values of 'i' are defined by the knot span and degree of the curve.
	glm::vec4 point_on_surface_in_homogeneous_space(std::uint32_t const k, std::vector<float> const& N, std::uint32_t const l, std::vector<float> const& M, std::vector<std::vector<glm::vec4> > const& P, std::uint32_t const p, std::uint32_t const q)
	{
		return glm::vec4{0.0};
	}
	...
	glm::vec4 point_on_surface_in_homogeneous_space(float u, float v, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& U, std::uint32_t p, std::vector<float> const& V, std::uint32_t q)
	{
		return glm::vec4{0.0};
	}
	...
	void derivative_control_grid_u(std::vector<std::vector<glm::vec4> >& dPu, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& U, std::uint32_t const p)
	{
	}
	...
	///     - Use dPu.push_back() to append Q_{i,j}^w to the end of the vector.
	///     - The implementation should likely contain a nested loop (inner and outer 'for' loop) for rows and columns
	void derivative_control_grid_v(std::vector<std::vector<glm::vec4> >& dPv, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& V, std::uint32_t const q)
	{
	}
	...
	glm::vec4 derivative_of_surface_u(float const u, float const v, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& U, std::uint32_t const p, std::vector<float> const& V, std::uint32_t const q,
	...
	///     - There is no loop in the implementation.
	///     - Use function @brief derivative_using_A and also @brief point_on_surface_in_homogeneous_space (twice).
	glm::vec4 derivative_of_surface_v(float const u, float const v, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& U, std::uint32_t const p, std::vector<float> const& V, std::uint32_t const q,
	...
	void grid_of_surface_points(
		std::uint32_t const points_u, std::uint32_t const points_v, std::vector<std::vector<glm::vec4> >& G, std::vector<std::vector<glm::vec4> > const& P, std::vector<float> const& U, std::uint32_t p, std::vector<float> const& V, std::uint32_t q)
	{
		...
		for (std::uint32_t i = 0U; i < points_u; ++i)
		{
			...
			for (std::uint32_t j = 0U; j < points_v; ++j)
			{
				...
				G.back().push_back(transform_point_from_homogeneous_space(point_on_surface_in_homogeneous_space(u, v, P, U, p, V, q)));
			}
		}
	}
	...
	/// NOTES:
	///     - Surface parameters u,v are regularly distributed over the entire domain of the surface.
	void grid_of_surface_derivatives(std::uint32_t const vectors_u, std::uint32_t const vectors_v, std::vector<std::vector<glm::vec4> >& dSu, std::vector<std::vector<glm::vec4> >& dSv, std::vector<std::vector<glm::vec4> > const& P,
		std::vector<float> const& U, std::uint32_t p, std::vector<float> const& V, std::uint32_t q)
	{
